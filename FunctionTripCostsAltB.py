...
def hotel_cost(nights):
    x = 140 * nights
    print "Hotel cost is: {0}".format(x)
    return x
...
def trip_cost(city, days, spending_money):
    sm = float(spending_money) * days
    totalCost = hotel_cost(days) + plane_ride_cost(city) + rental_car_cost(days) + sm
    print "Total trip price to {0}, including {1} for spending money is: {2}".format(city, sm, totalCost)
    return totalCost

trip_cost("Charlotte", 10, 50.25)
trip_cost("Tampa", 10, 50.89)
trip_cost("Pittsburgh", 10, 51.75)   
